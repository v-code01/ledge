...
async fn git_push_delta_pack_roundtrips() {
    ...
    let pack_dir = String::from_utf8(
        git(&["rev-parse", "--git-path", "objects/pack"], src.path())
            .await
            .stdout,
    )
    .unwrap()
    .trim()
    .to_string();
    let pack_file = std::fs::read_dir(src.path().join(&pack_dir))
        .unwrap()
    ...
    let verify = git(
        &["verify-pack", "-v", pack_file.to_str().unwrap()],
        src.path(),
    )
    .await;
    ...
    assert_git_ok(
        &git(&["remote", "add", "ledge", &remote_url], src.path()).await,
        "remote add",
    );
    ...
    assert_eq!(
        got, want,
        "pushed-then-cloned main SHA-1 matches (delta pack ingested correctly)"
    );
}
