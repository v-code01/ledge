...
use ledge_core::{ObjectId, RefName, HLC};
...
async fn test_concurrent_writes_64_tasks_disjoint_prefixes() {
    ...
    let store =
        Arc::new(RefStoreImpl::open(dir.path().to_path_buf(), Arc::new(HLC::new())).unwrap());
...
async fn test_concurrent_cas_single_ref() {
    ...
    for task_id in 0u16..16 {
        ...
        set.spawn(async move {
            ...
            loop {
                attempts += 1;
                if attempts > 10000 {
                    panic!("task {task_id} exceeded retry limit");
                }
                ...
                match store
                    .update(&name, new_oid, Some(current_entry.target))
                    .await
                {
                    Ok(_) => break,
    ...
    let final_entry = store
        ...
        .await
        .unwrap()
        .expect("ref missing after concurrent CAS");
    assert_eq!(
        final_entry.version, 17,
        "expected 17 versions (1 create + 16 updates), got {}",
        final_entry.version
    );
}
...
async fn test_wal_recovery_arbitrary_truncation() {
    let dir = tempdir().unwrap();
    let written: Vec<(String, ObjectId)> = (0u8..20)
        .map(|i| (format!("refs/heads/branch{i:02}"), make_oid(i)))
        .collect();
    {
        let store = RefStoreImpl::open(dir.path().to_path_buf(), Arc::new(HLC::new())).unwrap();
        for (name_str, oid) in &written {
            store
                .update(&RefName::new(name_str).unwrap(), *oid, None)
                .await
                .unwrap();
        }
    }
    ...
    assert!(
        !all.is_empty(),
        "at least some refs must survive truncation recovery"
    );
...
async fn test_wal_compaction_then_recovery() {
    ...
    let refs_to_write = 30u8;
    {
        ...
        for i in 0..refs_to_write {
            store
                .update(
                    &RefName::new(&format!("refs/heads/r{i:02}")).unwrap(),
                    make_oid(i),
                    None,
                )
                .await
        ...
        for i in refs_to_write..refs_to_write + 5 {
            store
                .update(
                    &RefName::new(&format!("refs/heads/post{i}")).unwrap(),
                    make_oid(i),
                    None,
                )
                .await
    ...
    for i in 0..refs_to_write {
        let entry = store2
            .get(&RefName::new(&format!("refs/heads/r{i:02}")).unwrap())
            .await
            .unwrap()
    ...
    for i in refs_to_write..refs_to_write + 5 {
        let entry = store2
            .get(&RefName::new(&format!("refs/heads/post{i}")).unwrap())
            .await
            .unwrap()
...
async fn test_background_compaction_triggers_on_threshold() {
    let dir = tempdir().unwrap();
    let store = Arc::new(
        RefStoreImpl::open_with_compaction_threshold(
    ...
    assert!(
        wal_size < 4096,
        "WAL size {wal_size} after compaction should be < 4096 bytes"
    );
    for i in 0u8..20 {
        let name = RefName::new(&format!("refs/heads/compact{i}")).unwrap();
        let entry = store
            ...
            .await
            .unwrap()
            .unwrap_or_else(|| panic!("ref compact{i} missing"));
        assert_eq!(entry.target, make_oid(i));
    }
}
